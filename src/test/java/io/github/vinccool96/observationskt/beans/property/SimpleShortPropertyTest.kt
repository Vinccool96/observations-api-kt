...
class SimpleShortPropertyTest {

    @Test
    fun testConstructor_NoArguments() {
        val v: ShortProperty = SimpleShortProperty()
    ...
    fun testConstructor_InitialValue() {
        ...
        val v2: ShortProperty = SimpleShortProperty(DEFAULT_VALUE)
    ...
    fun testConstructor_Bean_Name() {
        val bean = Any()
        val name = "My name"
        val v: ShortProperty = SimpleShortProperty(bean, name)
        assertEquals(bean, v.bean)
        assertEquals(name, v.name)
        assertEquals(DEFAULT_VALUE, v.get())

        val v2: ShortProperty = SimpleShortProperty(bean, null)
    ...
    fun testConstructor_Bean_Name_InitialValue() {
        val bean = Any()
        val name = "My name"
        val v1: ShortProperty = SimpleShortProperty(bean, name, VALUE_1)
        assertEquals(bean, v1.bean)
        assertEquals(name, v1.name)
        assertEquals(VALUE_1, v1.get())

        val v2: ShortProperty = SimpleShortProperty(bean, name, DEFAULT_VALUE)
        assertEquals(bean, v2.bean)
        assertEquals(name, v2.name)
        assertEquals(DEFAULT_VALUE, v2.get())

        val v3: ShortProperty = SimpleShortProperty(bean, null, VALUE_1)
        assertEquals(bean, v3.bean)
        assertEquals(null, v3.name)
        assertEquals(VALUE_1, v3.get())

        val v4: ShortProperty = SimpleShortProperty(bean, null, DEFAULT_VALUE)
    ...
    companion object {
        ...
        private const val VALUE_1: Short = -4354
