...
class SimpleLongPropertyTest {
    ...
    fun testConstructor_NoArguments() {
        val v: LongProperty = SimpleLongProperty()
    ...
    fun testConstructor_InitialValue() {
        ...
        val v2: LongProperty = SimpleLongProperty(DEFAULT_VALUE)
    ...
    fun testConstructor_Bean_Name() {
        val bean = Any()
        val name = "My name"
        val v: LongProperty = SimpleLongProperty(bean, name)
        assertEquals(bean, v.bean)
        assertEquals(name, v.name)
        assertEquals(DEFAULT_VALUE, v.get())

        val v2: LongProperty = SimpleLongProperty(bean, null)
    ...
    fun testConstructor_Bean_Name_InitialValue() {
        val bean = Any()
        val name = "My name"
        val v1: LongProperty = SimpleLongProperty(bean, name, VALUE_1)
        assertEquals(bean, v1.bean)
        assertEquals(name, v1.name)
        assertEquals(VALUE_1, v1.get())

        val v2: LongProperty = SimpleLongProperty(bean, name, DEFAULT_VALUE)
        assertEquals(bean, v2.bean)
        assertEquals(name, v2.name)
        assertEquals(DEFAULT_VALUE, v2.get())

        val v3: LongProperty = SimpleLongProperty(bean, null, VALUE_1)
        assertEquals(bean, v3.bean)
        assertEquals(null, v3.name)
        assertEquals(VALUE_1, v3.get())

        val v4: LongProperty = SimpleLongProperty(bean, null, DEFAULT_VALUE)
    ...
    companion object {
        ...
        private const val VALUE_1 = -4354L
