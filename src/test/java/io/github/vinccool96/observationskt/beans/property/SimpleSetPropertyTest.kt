...
class SimpleSetPropertyTest {
    ...
    fun testConstructor_NoArguments() {
        val v: SetProperty<Any> = SimpleSetProperty()
    ...
    fun testConstructor_InitialValue() {
        ...
        val v2: SetProperty<Any> = SimpleSetProperty(DEFAULT_VALUE)
    ...
    fun testConstructor_Bean_Name() {
        val bean = Any()
        val name = "My name"
        val v1: SetProperty<Any> = SimpleSetProperty(bean, name)
        assertEquals(bean, v1.bean)
        assertEquals(name, v1.name)
        assertEquals(DEFAULT_VALUE, v1.get())

        val v2: SetProperty<Any> = SimpleSetProperty(bean, null)
    ...
    fun testConstructor_Bean_Name_InitialValue() {
        val bean = Any()
        val name = "My name"
        val v1: SetProperty<Any> = SimpleSetProperty(bean, name, VALUE_1)
        assertEquals(bean, v1.bean)
        assertEquals(name, v1.name)
        assertEquals(VALUE_1, v1.get())

        val v2: SetProperty<Any> = SimpleSetProperty(bean, name, DEFAULT_VALUE)
        assertEquals(bean, v2.bean)
        assertEquals(name, v2.name)
        assertEquals(DEFAULT_VALUE, v2.get())

        val v3: SetProperty<Any> = SimpleSetProperty(bean, null, VALUE_1)
        assertEquals(bean, v3.bean)
        assertEquals(null, v3.name)
        assertEquals(VALUE_1, v3.get())

        val v4: SetProperty<Any> = SimpleSetProperty(bean, null, DEFAULT_VALUE)
    ...
    companion object {
        ...
        private val DEFAULT_VALUE: ObservableSet<Any>? = null

        private val VALUE_1: ObservableSet<Any> = ObservableCollections.observableSet(Any())
