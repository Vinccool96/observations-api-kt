 ...
 *
 * `LongBinding` provides a simple invalidation-scheme. An extending class can register dependencies by calling [bind].
 * If one of the registered dependencies becomes invalid, this `LongBinding` is marked as invalid. With [unbind]
 ...
 * @see LongExpression
 */
abstract class LongBinding : LongExpression(), NumberBinding {
     ...
     * @return the current value
     */
    protected abstract fun computeValue(): Long

    /**
     * Returns a string representation of this `LongBinding` object.
     *
     * @return a string representation of this `LongBinding` object.
     */
    override fun toString(): String {
        return if (this.validState) "LongBinding [value: ${get()}]" else "LongBinding [invalid]"
    }
