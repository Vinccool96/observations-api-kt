 ...
 */
class ObservableLongArrayImpl() : ObservableArrayBase<ObservableLongArray>(), ObservableLongArray {
     ...
     */
    constructor(src: ObservableLongArray) : this() {
        setAll(src)
    }
    ...
    private fun addAllInternal(src: LongArray, startIndex: Int, endIndex: Int) {
    ...
    private fun addAllInternal(src: ObservableLongArray, startIndex: Int, endIndex: Int) {
        val length = endIndex - startIndex
        growCapacity(length)
        src.copyInto(this.array, this.sizeState, startIndex, endIndex)
        this.sizeState += length
    ...
    override fun addAll(src: ObservableLongArray) {
    ...
    override fun addAll(src: ObservableLongArray, startIndex: Int, endIndex: Int) {
        rangeCheck(src, startIndex, endIndex)
        addAllInternal(src, startIndex, endIndex)
    }

    private fun setAllInternal(src: LongArray, startIndex: Int, endIndex: Int) {
        val length = endIndex - startIndex
    ...
    private fun setAllInternal(src: ObservableLongArray, startIndex: Int, endIndex: Int) {
        val length = endIndex - startIndex
        val sizeChanged = this.size != length
        if (src === this) {
            if (startIndex == 0) {
    ...
    override fun setAll(src: ObservableLongArray) {
    ...
    override fun setAll(src: ObservableLongArray, startIndex: Int, endIndex: Int) {
        rangeCheck(src, startIndex, endIndex)
        setAllInternal(src, startIndex, endIndex)
    }
    ...
    override fun set(src: ObservableLongArray, destinationOffset: Int, startIndex: Int, endIndex: Int) {
        val length = endIndex - startIndex
        rangeCheck(destinationOffset + length)
        src.copyInto(this.array, destinationOffset, startIndex, endIndex)
        fireChange(false, destinationOffset, destinationOffset + length)
    ...
    override fun copyInto(destination: ObservableLongArray, destinationOffset: Int, startIndex: Int, endIndex: Int) {
        rangeCheck(endIndex)
    ...
    private fun rangeCheck(src: LongArray, startIndex: Int, endIndex: Int) {
    ...
    private fun rangeCheck(src: ObservableLongArray, startIndex: Int, endIndex: Int) {
        if (startIndex < 0 || endIndex > src.size) {
            throw ArrayIndexOutOfBoundsException(src.size)
        }
        if (endIndex < startIndex) {
